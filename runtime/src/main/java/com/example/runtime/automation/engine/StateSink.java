...
/**
 * Результаты тактов → база и мониторы.
 * <ul>
...
public class StateSink implements TaskObserver {
    ...
    public void status(TaskStatusUpdate update) {
        ...
        synchronized (this) {
            ...
            boolean refresh = previous != null && previous.lastRunAtMs() != null && update.lastRunAtMs() != null
                    && update.lastRunAtMs() - previous.lastRunAtMs() >= properties.getStatusRefreshMs();
            if (!changed && !refresh) {
                return;
            }
    ...
    @Override
    public synchronized void checkpoint(long projectId, long taskId, String definitionHash,
                                        Map<String, Object> state) {
        checkpoints.put(projectId + ":" + taskId,
                new AutomationStore.CheckpointRow(projectId, taskId, definitionHash, state));
    }
        ...
        try {
            ...
            synchronized (this) {
                ...
                if (now - lastFailureLogMs > 30_000) {
                    ...
                    log.warn("Состояние фоновых задач не сохранено, задачи работают из памяти: {}", e.getMessage());
                }
            }
        }
    }
