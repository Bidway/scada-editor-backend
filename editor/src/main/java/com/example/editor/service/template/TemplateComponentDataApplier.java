 ...
 * <p>
 * Строки с уже известным именем правятся на месте, а не пересоздаются. Ключ — имя (плюс номер
 * среди одноимённых), а не id: вложенные DTO ({@code ScriptCreateDto}, {@code ComponentStateDto})
 * общие со сценой и поле {@code id} несут, но здесь оно <b>намеренно не читается</b> (scada-6n7).
 * У сцены чужой вложенный id — 400; у шаблона — не ошибка: восстановление версии присылает снимок
 * с id, и после удаления строки в нём был бы «чужой» номер. Читать id начнём вместе со слиянием
 * шаблонов; до тех пор асимметрию держит тест {@code TemplateSaveIT.nestedIdInTemplate_isIgnored_rowMatchedByName}. Тот же приём, что у свойств,
