...
public abstract class EditorApiTestSupport extends PostgresTestContainerSupport {
    ...
    /** Удаление компонентов без проверки статуса ответа — вызывающий сам решает, что ожидать. */
    protected ResultActions deleteComponents(List<Long> ids, Integer basedOnVersion) throws Exception {
        StringBuilder body = new StringBuilder("{\"ids\":").append(objectMapper.writeValueAsString(ids));
        if (basedOnVersion != null) {
            body.append(",\"based_on_version\":").append(basedOnVersion);
        }
        body.append("}");
        return mockMvc.perform(delete("/api/editor/components")
                .header("X-Username", USER)
                .contentType(MediaType.APPLICATION_JSON)
                .content(body.toString()));
    }
