...
class UndoIT extends ChannelApiTestSupport {
    ...
    /**
     * Повторная отмена той же записи отклоняется: undoneAt уже проставлен. Проверяется
     * последовательный случай; параллельный (двойной клик, два запроса одновременно) держит
     * блокировка строки журнала в findByIdForUpdate (scada-wjd).
     */
    @Test
